class Board {
     ...
     * @returns {boolean}
     */
    isNextStepToSelf(nextCellCoords) {
        let nextCell = this.getCellEl(nextCellCoords.x, nextCellCoords.y);
...
class Game {
    ...
    doTick() {
        ...
        if (this.isGameLost(this.snake.newHeadCoords)) {
            return;
        }
        if (this.isGameWon()) {
            return;
        ...
        if (this.board.isHeadOnFood()) {
            this.setScore(this.snake.body.length);
     ...
     * @returns {boolean} если мы съели сами себя(если мы шагнули в стену), тогда
     * true, иначе false.
     */
    isGameLost(newHeadCoords) {
        if (this.board.isNextStepToSelf(this.snake.body[0])) {
            clearInterval(this.tickIdentifier);
...
class Snake {
    ...
    performStep() {
        ...
        switch (this.direction) {
            case "down":
                if (currentHeadCoords.y === this.settings.colsCount) {
                    newHeadCoords.y = 1;
                    break;
                }
                newHeadCoords.y++;
            ...
            case "left":
                ...
                break;
            case "right":
                if (currentHeadCoords.x === this.settings.rowsCount) {
                    newHeadCoords.x = 1;
                    break;
                }
                newHeadCoords.x++;
